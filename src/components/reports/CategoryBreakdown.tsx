...
export function CategoryBreakdown() {
  ...
  const expenseCategories = insights.filter(
    i => !mockCategories.find(c => c.id === i.category_id)?.is_income
  );

  const maxAmount = Math.max(...expenseCategories.map(i => i.total_amount));
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {expenseCategories.map(insight => {
          const category = mockCategories.find(c => c.id === insight.category_id);
                ...
                <div className="flex items-center gap-2">
                  ...
                  {insight.trend === 'down' && (
                    <div className="flex items-center text-income text-xs">
                      ...
                      {Math.abs(insight.trend_percentage || 0)}%
                    </div>
                  )}
